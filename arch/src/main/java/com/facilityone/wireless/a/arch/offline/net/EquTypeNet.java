 ...
 * description: 设备类型网络请求
 * Date: 2018/10/18 3:24 PM
 */
public class EquTypeNet {

    public static void requestEquType(final DownloadReq req, final OnDownloadListener listener) {
        OkGo.<BaseResponse<DownloadResp<EquTypeEntity>>>post(FM.getApiHost() + OfflineUri.EQU_TYPE_URL)
                .upJson(GsonUtils.toJson(req, false))
                .isSpliceUrl(true)
                .execute(new FMJsonCallback<BaseResponse<DownloadResp<EquTypeEntity>>>() {
                    @Override
                    public void onSuccess(Response<BaseResponse<DownloadResp<EquTypeEntity>>> response) {
                        final DownloadResp<EquTypeEntity> data = response.body().data;
                        ...
                        final List<EquTypeEntity> equTypeEntities = data.contents;
                        final Page page = data.page;
                        Observable.create(new ObservableOnSubscribe<Boolean>() {
                            @Override
                            public void subscribe(ObservableEmitter<Boolean> emitter) throws Exception {
                                EquTypeDao equTypeDao = new EquTypeDao();
                                boolean add = equTypeDao.addEquType(equTypeEntities);
                                emitter.onNext(add);
                                emitter.onComplete();
                            }
                        })
                    ...
                    @Override
                    public void onError(Response<BaseResponse<DownloadResp<EquTypeEntity>>> response) {
